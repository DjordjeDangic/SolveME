...
def gaussian(x, x0, smearing):
    ...
    return np.exp(-0.5*(x1-x2)**2/smearing**2)/np.sqrt(2.0*np.pi)/smearing
...
class mesolver:
    ...
    def load(self, dyn_filename = None, nqirr = 1, elphdyn_filename = None, wee_filename = None):
        ...
        if(wee_filename is not None):
            freq, energy, dos, re_wee, im_wee = read_wee(wee_filename)
            self.wee_freq = freq/RY_TO_EV
            self.wee_energy = energy/RY_TO_EV
            self.wee_dos = dos*RY_TO_EV
            self.wee = (re_wee + 1j*im_wee)/RY_TO_EV
    ...
    def calculate_a2f(self, anharmonic = False, comm_dyn_filename = None, comm_nqirr = 1, third_order_filename = None, nom = 2000, scattering_mesh = [10,10,10], mode_mixing = 'no', on_full_grid = False, temperature = 0.0, a2f_smearing = 0.5):
        ...
        if(anharmonic):
            ...
            comm_supercell_matrix = comm_dyn.GetSupercell()
            fc3 = CC.ForceTensor.Tensor3(comm_dyn.structure, comm_dyn.structure.generate_supercell(comm_supercell_matrix), comm_supercell_matrix)
        tc = CC.ThermalConductivity.ThermalConductivity(self.dyn, fc3, kpoint_grid = supercell_matrix, scattering_grid = scattering_mesh, smearing_scale = 1.0, smearing_type = 'constant', cp_mode = 'quantum', off_diag = False, phase_conv = 'step')
        temperature = temperature
        tc.setup_harmonic_properties(0.00001)
        ...
        if(anharmonic):
            ...
            for ikpt in range(len(qpt_id1)):
                self.pq_dos[ikpt] = float(self.elph_weights[ikpt]) * gaussian(self.a2f_omega, tc.freqs[qpt_id1[ikpt]], smearing)
        ...
        self.pdos = self.pdos/float(np.sum(self.elph_weights) - 1)
    ...
    def write_a2f(self, smear_id = 0, a2f_filename = 'a2f'):
        ...
        with open(a2f_filename, 'w+') as outfile:
            ...
            outfile.write('# Frequency (1/cm)          alpha2f             Phonon DOS (cm)  \n')
            for i in range(len(self.a2f_omega)):
                outfile.write(3*' ' + format(self.a2f_omega[i]*RY_TO_CM, '.12f'))
                outfile.write(3*' ' + format(self.a2f[smear_id, i], '.12f'))
                outfile.write(3*' ' + format(self.pdos[i]/RY_TO_CM, '.12f') + '\n')
    ...
    def get_omega_log(self, units = '1/cm', smear_id = 0):
        ...
        if(units == '1/cm'):
            omegalog = omegalog*RY_TO_CM
        elif(units == 'K'):
            omegalog = omegalog*RY_TO_EV/K_B
        else:
    ...
    def get_initial_gap_estimate(self, mu_star = 0.15, smear_id = 0):
        ...
        return T_ad*3.52/2.0*K_B/RY_TO_EV
    ...
    def solve_isotropic(self, T_start = 1.0, T_end = 300.0, wcut_ratio = 10.0, mu_star_approx = True, ntemp = 2, mu_star = 0.15, gap_filename = 'SC_GAP', log = False, smear_id = 0, constant_dos = False):
        ...
        for i in range(ntemp):
            ...
            if(not gap_is_zero):
                temp_ry = self.temperatures[i]*K_B/RY_TO_EV
                if(i == 0):
                    delta0 = self.get_initial_gap_estimate(mu_star, smear_id = smear_id)
                    delta_init = delta0
                    if(mu_star_approx):
                        w_i, delta_i, z_i, index = self.solve_isotropic_at_T_mu_star_one_sided(mu_star, w_cut, temp_ry, delta0, log, smear_id = smear_id)
                    else:
                        if(constant_dos):
                            w_i, delta_i, z_i, phi_c, index = self.solve_isotropic_at_T_constant_dos_one_sided(w_cut, temp_ry, delta0, starting_Ne, log, smear_id = smear_id)
                        else:
                            w_i, delta_i, z_i, chi_i, phi_c, index, ef = self.solve_isotropic_at_T(w_cut, temp_ry, delta0, starting_Ne, log, smear_id = smear_id)
                else:
        ...
        with open(gap_filename, 'w+') as outfile:
            ...
            for i in range(ntemp):
                ...
                if(mu_star_approx):
                    outfile.write(8*' ' + format(self.delta[i][self.indices[i]]*RY_TO_EV*1.0e3, '.12f') + '\n')
                else:
                    if(not constant_dos):
                        for j in range(1, len(self.wee_energy)):
                            if(self.wee_energy[j] > self.ef[i] and self.wee_energy[j-1] < self.ef[i]):
                    ...
                    outfile.write(8*' ' + format(gap*RY_TO_EV*1.0e3, '.12f') + '\n')
    ...
    def solve_isotropic_at_T_mu_star(self, mu_star, w_cut, T, delta0, log = False, smear_id = 0):
        ...
        diff = 1.0
        w = self.get_matsubara_frequencies(w_cut, T)
        index = int(len(w)/2) + 1
        ...
        delta1[index] = delta0
        delta1[index - 1] = delta0
        z1 = np.ones(len(w))
        z1[index] = 2.0
        z1[index-1] = 2.0
        iteration = 1
        lnm = self.generate_Lnm_matrix(w, smear_id = smear_id)
        lnm_mu = lnm - mu_star
    ...
    def solve_isotropic_at_T_mu_star_one_sided(self, mu_star, w_cut, T, delta0, log = False, smear_id = 0):
    ...
    def solve_isotropic_at_T(self, w_cut, T, delta0, Ne, log = False, smear_id = 0):
        ...
        Nef = 0.0
        ief = 0
        for i in range(len(self.wee_dos)):
            if(abs(self.wee_energy[i]) < 1.0e-6):
                Nef = self.wee_dos[i]
                ief = i
                break
        if(Nef == 0.0):
        ...
        mu_star = self.wee_dos[ief]*self.wee[0,ief,ief].real/5.0
        w0, delta1, z0, index0 = self.solve_isotropic_at_T_mu_star_one_sided(mu_star, w_cut, T, delta0, log = log, smear_id = smear_id)
        if(np.all(w0 == w)):
            phi1 = delta1*z0
            z1 = z0
            ...
            phi_c = -2.0*np.einsum('i,ji->j', self.wee_dos * term2, self.wee[0].real) * de_wee_energy
        else:
            raise RuntimeError('This should not happen!')

        while((diff > self.thr and iteration < self.max_iter and abs(phi1[index]/z1[index]) > 1.0e-6/RY_TO_EV) or iteration < int(self.starting_Coulomb_weight) + 1):
            ...
            phi_c = -coulomb_weight*np.einsum('i,ij->j', self.wee_dos * (term1 + term2), self.wee[0].real) * de_wee_energy

            ef = brentq(fermi_root, -0.1/RY_TO_EV + ef, 0.1/RY_TO_EV + ef, args=(self.wee_energy, self.wee_dos, T, w, chi2, theta, Ne), xtol=1e-6)

            diff = np.linalg.norm(phi1 - phi2)/np.linalg.norm(phi2)
            print(iteration, phi1[index], phi2[index], abs(phi1[index] - phi2[index]), z1[index], phi_c[ief], term1[ief], term2[ief], theta[ief,index], sqrt_term[ief], de_wee_energy, diff, self.wee_energy[ief], ef)
    ...
    def solve_isotropic_at_T_constant_dos(self, w_cut, T, delta0, Ne, log = False, smear_id = 0):
        ...
        for i in range(1, len(w)):
            if(w[i] > 0.0 and w[i-1] < 0.0):
                index = i
                break
        phi1 = np.zeros(len(w), dtype=float)
        phi1[index] = delta0
        phi_c = np.zeros(len(self.wee_dos), dtype=float) 
        z1 = np.ones(len(w), dtype=float) 
        z1[index] = 2.0
        iteration = 1
        lnm = self.generate_Lnm_matrix(w, smear_id = smear_id)
        theta = np.zeros((len(self.wee_dos), len(w)))
        ef = 0.0
        de_wee_energy = (self.wee_energy[1] - self.wee_energy[0])
        Nef = 0.0
        ief = 0
        ...
        if(Nef == 0.0):
            raise RuntimeError('Density of states at the Fermi level is 0.0!')
        mu_star = self.wee_dos[ief]*self.wee[0,ief,ief].real/5.0
        w0, delta1, z0, index = self.solve_isotropic_at_T_mu_star(mu_star, w_cut, T, delta0, log = log, smear_id = smear_id)
        if(np.all(w0 == w)):
            phi1 = delta1*z0
            z1 = z0
            phi_c = -2.0 * T * np.dot(self.wee_dos * np.sum(delta1[:,np.newaxis]/(w0[:,np.newaxis]**2 + self.wee_energy[np.newaxis,:]**2 + delta1[:,np.newaxis]**2), axis=0), self.wee[0].real) * de_wee_energy 
        else:
            raise RuntimeError('This should not happen!')
        while((diff > self.thr and iteration < self.max_iter and abs(phi1[index]/z1[index]) > 1.0e-6/RY_TO_EV) or iteration < int(self.starting_Coulomb_weight)):
            ...
            tot_phi = phi2[np.newaxis,:] + phi_c[:, np.newaxis]
            theta = np.maximum(((w*z2)[np.newaxis, :])**2 + (self.wee_energy[:, np.newaxis])**2 + tot_phi**2, 1.0e-12)

            z1 = 1.0 + np.pi*T/w * np.einsum('j,ij ->i',w*z2/np.sqrt(theta[ief]), lnm) 
            #theta = (w*z1)[np.newaxis, :]**2 + self.wee_energy[:, np.newaxis]**2 + tot_phi**2

            phi1 =  T * np.pi * np.einsum('j,ij ->i', tot_phi[ief]/np.sqrt(theta[ief]), lnm)
            #tot_phi = phi1[np.newaxis,:] + phi_c[:, np.newaxis]
            ...
            if(len(phi1)%2 == 0):
                ...
                phi1[:-1] += np.flipud(phi1[:-1])
                phi1[:-1] /= 2.0
                z1[:-1] += np.flipud(z1[:-1])
                z1[:-1] /= 2.0
            """

            sqrt_term = np.maximum(np.sqrt(phi_c ** 2 + self.wee_energy** 2), 1.0e-12).copy()
            term1 = (phi_c/2.0 * np.tanh(sqrt_term/2.0/T)/sqrt_term).copy()
            term2 = np.zeros_like(term1)
            term2 =  (T * np.sum(tot_phi/theta - phi_c[:, np.newaxis]/((w[np.newaxis,:])**2 + (sqrt_term[:, np.newaxis])**2), axis=1)).copy()
            term3 =  T * np.sum(tot_phi/theta, axis=1)
            term4 =  T * np.sum(-phi_c[:, np.newaxis]/((w[np.newaxis,:])**2 + (sqrt_term[:, np.newaxis])**2), axis=1)
            phi_c = -coulomb_weight * np.einsum('i,ji->j', self.wee_dos * (term1 + term2), self.wee[0].real) * de_wee_energy / 2.0

            diff = np.linalg.norm(phi1 - phi2)/np.linalg.norm(phi2)
            #print(iteration, phi1[index], phi2[index], abs(phi1[index] - phi2[index]), z1[index], phi_c[ief], term1[ief], term2[ief], term3[ief], term4[ief], theta[ief,index], sqrt_term[ief], de_wee_energy, diff)
    ...
    def solve_isotropic_at_T_constant_dos_one_sided(self, w_cut, T, delta0, Ne, log = False, smear_id = 0):
        ...
        ief = 0
        for i in range(len(self.wee_dos)):
            if(self.wee_energy[i] <= 0.0 and self.wee_energy[i + 1] > 0.0):
                Nef = self.wee_dos[i]
                ief = i
                break
        if(Nef == 0.0):
        ...
        mu_star = 0.0 # self.wee_dos[ief]*self.wee[0,ief,ief].real/5.0
        w0, delta1, z0, index0 = self.solve_isotropic_at_T_mu_star_one_sided(mu_star, w_cut, T, delta0, log = log, smear_id = smear_id)
        if(np.all(w0 == w)):
            phi1 = delta1*z0#*4.0
            z1 = z0
            theta = np.maximum(((w*z1)[np.newaxis, :])**2 + (self.wee_energy[:, np.newaxis])**2 + phi1**2, 1.0e-12)
            term2 =  T * np.sum(phi1[np.newaxis, :]/theta, axis=1)
            phi_c = -0.1*np.einsum('i,ji->j', self.wee_dos * term2, self.wee[0].real) * de_wee_energy
        else:
            raise RuntimeError('This should not happen!')
        print(iteration - 1, phi1[index], phi1[index], abs(phi1[index] - phi1[index]), z1[index], phi_c[ief], 0.0, term2[ief], term2[ief], 0.0, theta[ief,index], 0.0, de_wee_energy, 0.0, self.wee_energy[ief])
        while((diff > self.thr and iteration < self.max_iter and abs(phi1[index]/z1[index]) > 1.0e-6/RY_TO_EV) or iteration < int(self.starting_Coulomb_weight*1.1)):
            ...
            z1 = 1.0 + np.pi*T/w * np.einsum('j,ji ->i',w*z2/np.sqrt(theta[ief]), lm) 

            phi1 =  T * np.pi * np.einsum('j,ji ->i', tot_phi[ief]/np.sqrt(theta[ief]), lp)
            ...
            term3 =  T * np.sum(tot_phi/theta, axis=1)
            term4 =  T * np.sum(-phi_c[:, np.newaxis]/((w[np.newaxis,:])**2 + (sqrt_term[:, np.newaxis])**2), axis=1)
            phi_c = -coulomb_weight * np.einsum('i,ji->j', self.wee_dos * (term1 + term2), self.wee[0].real) * de_wee_energy
            diff = np.linalg.norm(phi1 - phi2)/np.linalg.norm(phi2)
            print(iteration, phi1[index], phi2[index], abs(phi1[index] - phi2[index]), z1[index], phi_c[ief], term1[ief], term2[ief], term3[ief], term4[ief], theta[ief,index], sqrt_term[ief], de_wee_energy, diff, self.wee_energy[ief])
            ...
            if(self.aa):
                ...
                if(len(history_f) >= 2):#self.anderson):
                    F = np.array(history_f).T
                    F =  F / np.linalg.norm(F, axis=0)
                    try:
                        alpha = np.linalg.lstsq(F, history_f[-1], rcond=None)[0]
